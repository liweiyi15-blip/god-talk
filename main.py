...
CHANNEL_MAPPING = {
    1478987667906760853: 1495712410898661448  # 源频道 -> 目标发布频道
}
...
def format_message(original_text):
    """提取原始消息内容并转换为极简格式"""
    # 动态抓取名字 (例如 [猫猫王] 或 [18倍佬])
    name_match = re.search(r'\[(.*?)\]仓位变动提醒', original_text)
    user_name = name_match.group(1) if name_match else "仓位"
    ...
    final_msg += f"[{user_name}]仓位变动提醒 ({now_str})\n\n"
...
async def on_message(message):
    ...
    if message.channel.id in CHANNEL_MAPPING:
        # 简单过滤：只处理包含关键标识符的消息
        if "变化类型" in full_text and "当前持仓明细" in full_text:
            try:
                # 转换格式
                formatted_msg = format_message(full_text)
                # 获取对应的目标频道并发送
                target_channel_id = CHANNEL_MAPPING[message.channel.id]
                target_channel = bot.get_channel(target_channel_id)
                if target_channel:
                    await target_channel.send(formatted_msg)
                    print(f"✅ 成功从 {message.channel.id} 转发到了 {target_channel_id}")
                else:
                    print(f"❌ 错误：找不到目标频道 {target_channel_id}")
